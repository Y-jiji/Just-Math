...
然而在开始并确信, 需要但没有创建的依赖要先创建. 

创建了但是没有写完的内容要添加上 `#写作中` 标签, 这点非常重要(复制空白模板文件即可)

当本文件已经修改完成, 但是下游文件没有修改时, 要添加上 `#下游未修改` 标签. 
...
使用字面值下标时, 用 $\$$ 来取代索引出现的位置, 之后正常使用该标识符即可. 

使用字面值下标的标识符通过 $X_{\$}$ 这样的格式进行声明, 表示使用字面值进行索引. 

一个变量的字面值只在其作用域内有效, 在作用域外无效. 

## 字面值下标的规范: 被定义项

在被定义项中使用字面值下标时, 使用形如 $X_{\$Y}$ 的记号来表示被定义项的某个位置参数使用的是字面值. 

例如拓扑空间一章中定义的 $\text{Interior}$ . 
